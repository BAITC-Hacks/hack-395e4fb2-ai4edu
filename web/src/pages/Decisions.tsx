...
export function Decisions({scenario:s,plan,name,onName,onChange,onRun,busy,mock,error}:{scenario:Scenario;plan:Decision[];name:string;onName:(v:string)=>void;onChange:(v:Decision[])=>void;onRun:()=>void;busy:boolean;mock:boolean;error:string}) {
  ...
  function changeDistrict(index:number,district_id:string) {
    ...
    onChange(next); setEditError(''); setFeedback('Район изменён. Бюджет и валидность подтвердит Go.');
  }
  ...
  return <>
    ...
    <div className="planner"><section className="catalog" aria-label="Каталог мероприятий" ref={catalog} tabIndex={-1}>
      ...
      <Recommendation scenario={s} mock={mock} busy={busy} onApply={next=>{onChange(next);setEditing(null);setEditError('');setFeedback('Рекомендованный план выбран. Бюджет проверяет Go.');}}/>
    </section>
    <aside className="plan-panel" id="plan" tabIndex={-1} aria-label="Выбранный план"><div className="plan-heading"><p className="eyebrow">ВАШ ПЛАН</p><strong>{plan.length} / {s.required_decisions}</strong></div>
      ...
      <div className="budget"><div><span>Использовано · Go</span><strong>{checked?.total_cost ?? '—'} <small>/ {s.budget}</small></strong></div>{checked&&<progress max={s.budget} value={Math.min(checked.total_cost,s.budget)} aria-label="Использованный бюджет"/>}<div><span>Осталось · Go</span><strong className={checked&&checked.remaining_budget<0?'danger':''}>{checked?`${checked.remaining_budget} ед.`:'—'}</strong></div></div>
      <div className="plan-validation" id="run-reason">{mock?<p>В mock-режиме стоимость плана и валидность не подтверждаются. Подключите Go API.</p>:assessment.checking?<Loading>Проверяем план в Go…</Loading>:checked&&!checked.valid?<ul>{checked.validation_errors?.map((e,i)=><li key={i}>{validationMessage(e)}</li>)}</ul>:ready?<p className="ready"><Check size={17}/> План подтверждён Go</p>:null}{!checked&&summary.errors.length>0&&<><p>Подсказки по каталогу:</p><ul>{summary.errors.map((e,i)=><li key={i}>{e.message}</li>)}</ul></>}</div>
      {assessment.error&&<ErrorBox message={assessment.error} retry={assessment.retry}/>}
      {error && <ErrorBox message={error}/>}
      <button className="cta" aria-disabled={!ready||busy} aria-describedby="run-reason" onClick={()=>ready&&!busy&&onRun()}>{busy?<Loading>Считаем в Go…</Loading>:<>Рассчитать результат <ArrowRight size={18}/></>}</button>
    ...
    <div className="plan-dock"><span><strong>{plan.length} / {s.required_decisions} решений{checked?` · ${checked.total_cost} ед.`:''}</strong>{checked?`Осталось ${checked.remaining_budget} ед.`:mock?'Mock · без проверки Go':'Бюджет проверяет Go'}</span><a href="#plan">План и запуск ↑</a></div>
  </>;
}
