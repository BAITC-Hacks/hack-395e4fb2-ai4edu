...
export function Layout({page,navigate,scenario,mock,onMode,children}:{page:Page;navigate:(p:Page)=>void;scenario?:Scenario;mock:boolean;onMode:()=>void;children:React.ReactNode}) {
  ...
  </aside><main id="main-content" tabIndex={-1}><header><div className="breadcrumb">Рабочее пространство <span>/</span><strong>{nav.find(n=>n.id===page)?.label}</strong></div><button className={`mode ${mock?'mock':''}`} onClick={onMode}>{mock?'Mock · без расчёта':'Go API'}<span>{mock?'Подключить API':'Режим разработки'}</span></button></header>
    <div className="content">{mock && <div className="notice mock-banner"><strong>MOCK — режим разработки без backend.</strong> Каталог — сохранённый снимок Go. Можно собрать план; расчёт и AI недоступны.</div>}{children}<footer><span>Аким на 5 часов · HackAlem</span><span>Синтетические данные · не прогноз для реального города</span></footer></div>
  </main></div>;
}
